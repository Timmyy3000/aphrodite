...
import { DesignContextV1, GetDesignContextInputV1, ListScreensInputV1, ScreenListV1, type GetDesignContextInputV1 as GetDesignContextInput } from './contracts/v1.js';
import { APHRODITE_VERSION } from './version.js';

function success(value: unknown) {
  // structuredContent is the authoritative bounded result. Keeping content empty
  // avoids serializing the same potentially large JSON payload a second time.
  return { content: [], structuredContent: value } as any;
}
...
export function createMcpServer(projectRoot = process.cwd()): McpServer {
  ...
  const server = new McpServer({ name: 'aphrodite', version: APHRODITE_VERSION }, { capabilities: { tools: {} } });
  server.registerTool('list_design_screens', { title: 'List design screens', description: 'List visible top-level screens from an imported local Figma document. Read the bounded result from structuredContent.', inputSchema: ListScreensInputV1, outputSchema: ScreenListV1 }, async raw => {
    try { return success(await store.listScreens(raw)); } catch (error) { return failure(error); }
  });
  server.registerTool('get_design_context', { title: 'Get design context', description: 'Resolve a local Figma node and return bounded implementation-oriented facts. Read the bounded result from structuredContent.', inputSchema: GetDesignContextInputV1, outputSchema: DesignContextV1 }, async raw => {
