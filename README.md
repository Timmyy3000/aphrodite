...
Version `0.2.4` normalizes the parser-native geometry, transform, paint, stack-layout, and typography fields found in canvas 106 documents. It also fixes rich-text style lookup and boundaries and removes duplicate MCP success payloads. The separate project-aware `design-to-code` skill is maintained in the shared skills repository.
...
The repository-owned Codex skill is [`skills/aphrodite/SKILL.md`](skills/aphrodite/SKILL.md); it teaches local MCP setup and the evidence contract. The project-aware `design-to-code` implementation loop is maintained separately in [Timmyy3000/skills](https://github.com/Timmyy3000/skills).

To install the Aphrodite skill for Codex, clone or download this repository and copy its skill directory into the host's skills directory. For example, on Windows PowerShell:
