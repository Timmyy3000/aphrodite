...
4. Run the GitHub-backed npx commands from the application project root. This does not require an npm publication or a repository checkout in the application:
   ...
   ```bash
   npx --yes github:Timmyy3000/aphrodite#v0.2.4 init --project .
   npx --yes github:Timmyy3000/aphrodite#v0.2.4 import /path/to/design.fig --project . --file-key FILEKEY --alias handoff --json
   ```
   ...
   ```powershell
   npx --yes github:Timmyy3000/aphrodite#v0.2.4 init --project .
   npx --yes github:Timmyy3000/aphrodite#v0.2.4 import ".\design.fig" --project . --file-key FILEKEY --alias handoff --json
   ```
   ...
   ```json
   {
     "mcpServers": {
       "aphrodite": {
         ...
         "args": [
           "--yes",
           "github:Timmyy3000/aphrodite#v0.2.4",
           "mcp",
           "--project",
...
2. Ask the user for the intended UI flow and behavior before implementation: the screen's place in the flow, what each control does, navigation/back behavior, interactive and animated states, responsive expectations, and any requirements that are not visible in Figma. Ask for missing assets or an explicit decision when the design evidence is ambiguous; do not silently invent behavior or substitute generic artwork.
3. Call `list_design_screens` with `{ "fileKey": "FILEKEY" }` or `{ "alias": "handoff" }` to verify the import and discover visible screens.
4. Call `get_design_context` for the requested frame. Prefer the copied URL when the import was registered with its file key:
...
5. Read the result from `structuredContent`. Successful responses intentionally have an empty `content` array to avoid duplicating the full JSON payload in the agent context. The advertised output schema is authoritative; do not wait for or recreate a text mirror.
6. Read `facts` as recorded design evidence: bounds, layout mode, padding, gaps, sizing, typography, fills, strokes, radii, component references, and asset mappings.
7. Read `guidance` as heuristics. Preserve its confidence and evidence; do not present a suggestion as a Figma fact.
8. Inspect the consuming project's existing components, tokens, typography, responsive conventions, and tracked asset directories before writing code.
9. Build semantic flexbox/grid structure that reproduces the recorded relationships. Use absolute positioning only when the evidence and surrounding design genuinely require it; do not translate every `x`/`y` coordinate into CSS.
10. For an asset marked `resolved`, copy the cache file into an existing tracked application asset directory. Verify the destination with `git check-ignore <destination>` and make sure it is not ignored. Never make application code depend on `.aphrodite/`.
11. Validate the implementation with the consuming project's tests, responsive checks, and rendered output. Do not use browser screenshots as a substitute for design evidence missing from Aphrodite; report the extraction gap so the local handoff can be improved.
...
MCP stdout is reserved for JSON-RPC; diagnostics belong on stderr. Responses are bounded and advertise versioned output schemas. Read successful results from `structuredContent`; `content` may be empty by design. Treat `truncation` as part of the result: if content was omitted, request a narrower subtree or a larger permitted budget rather than inventing missing details.

If the agent host exposes only text content and shows a blank successful result, upgrade its MCP adapter to one that supports `structuredContent`; do not recreate the full JSON payload as a text mirror.
...
Version `0.2.4` returns parser-native sizes and positions, compact paints, stack layout fields, and correct font/rich-text style runs. Its default query includes two descendant levels to preserve semantic structure without flooding context with deep vector internals.
...
- After upgrading Aphrodite across an importer-version change, re-run `import` so parser and asset fixes are applied. If the existing alias or file key is registered to the previous import ID, add `--replace-registration` to the import command.
